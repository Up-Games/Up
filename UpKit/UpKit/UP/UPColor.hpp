...
public:
    ...
    static constexpr uint32_t pack_premultiplied_pixel(uint8_t ch1, uint8_t ch2, uint8_t ch3, UPFloat alpha) {
        return up_to_255(ch1 * alpha) | (up_to_255(ch2 * alpha) << 8) | (up_to_255(ch3 * alpha) << 16) | (up_to_255(alpha) << 24);
    }
...
static constexpr UPFloat LAB_Xn = 0.950470f;
static constexpr UPFloat LAB_Yn = 1.0f;
static constexpr UPFloat LAB_Zn = 1.088830f;

static constexpr UPFloat LAB_t0 = 4.0f / 29.0f;
static constexpr UPFloat LAB_t1 = 6.0f / 29.0f;
static constexpr UPFloat LAB_t2 = 3.0f * LAB_t1 * LAB_t1;
...
{
    return (c > LAB_t3) ? (powf(c, 1.0f / 3.0f)) : (c / LAB_t2 + LAB_t0);
}
...
UP_STATIC_INLINE LABF to_labf(const RGBF &c)
{
    ...
    UPFloat y = xyz.y();
    UPFloat z = xyz.z();
    UPFloat l = 116.0f * y - 16.0f;
    UPFloat a = 500.0f * (x - y);
    UPFloat b = 200.0f * (y - z);
...
UP_STATIC_INLINE LCHF to_lchf(const LABF &in_c)
{
    ...
    UPFloat c = sqrtf(a * a + b * b);
    UPFloat h = (atan2f(b, a) * RAD2DEG + 360);
    if (h > 360) {
        h -= 360;
    }
    else if (h < 0) {
...
UP_STATIC_INLINE LABF to_labf(const LCHF &in_c)
{
    ...
    return LABF(l, cosf(h) * c, sinf(h) * c, in_c.alpha());
}
...
UP_STATIC_INLINE RGBF to_rgbf(const LABF &c)
{
    ...
    UPFloat y = (l + 16) / 116.0f;
    UPFloat x = isnan(a) ? y : y + a / 500.0f;
    UPFloat z = isnan(b) ? y : y - b / 200.0f;
