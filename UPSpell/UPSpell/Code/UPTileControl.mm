...
- (instancetype)_initWithTile:(const UP::Tile &)tile
{
    ...
    UIBezierPath *contentPath = layout_manager.tile_path_for_glyph(tile.glyph());
    [contentPath appendPath:layout_manager.tile_path_for_score(tile.score())];
    if (tile.multiplier() != 1) {
        [contentPath appendPath:layout_manager.tile_path_for_multiplier(tile.multiplier())];
    }
